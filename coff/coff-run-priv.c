...
printf("%s(): LookupPrivilegeValue() returned %d\n", __func__, bRes);
    if (!bRes) return FALSE;
    
//    bRes = OpenThreadToken(GetCurrentThread(), TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES, TRUE, &hToken); 
//printf("%s(): OpenThreadToken() returned %d\n", __func__, bRes);
    bRes = OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES, &hToken); 
printf("%s(): OpenProcessToken() returned %d\n", __func__, bRes);
    if (!bRes)
	{
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), LANG_SYSTEM_DEFAULT,
				(LPTSTR)err_buf, sizeof(err_buf) / sizeof(char), NULL);
		printf("[stderr] Error (%08X): %s\n", GetLastError(), err_buf);
		return FALSE;
	}

    bRes = GetTokenInformation(hToken, TokenPrivileges, NULL, 0, &dwLen);
printf("%s(): GetTokenInformation() returned %d\n", __func__, bRes);
    if (TRUE == bRes)
    {
        CloseHandle(hToken);
        return FALSE;
...
printf("%s(): Iterate through all the privileges and enable the one required == SE_DEBUG_NAME (%s)\n",
			__func__, SE_DEBUG_NAME);
    bRes = FALSE;
...
printf("%s(): AdjustTokenPrivileges() returned %d, break;\n", __func__, bRes);
            break;
        }
    }
...
int main(int argc, char **argv)
{
	...
	res = EnablePrivilege();
	printf("EnablePrivilege() to enable SE_DEBUG_NAME returned res = %d\n", res);
	printf("Press any key...\n");
	getchar();
