...
// Shellcode params
//unsigned param_size;
//unsigned __int8 *params;				// params BLOB


// Params starter
/*
#define PARAM_GENERIC		('-hah')
*/

/*
#define switch_endianness(a) ((a & 0xFF) << 24 | (a & 0xFF00) << 8 | (a & 0xFF0000) >> 8 | (a & 0xFF000000) >> 24)

// Convert parameter as received in argv[] to code as defined in PARAM_xxx
// Needed to prepare generic parameters for shellcodes
UINT32 param_to_code(char *prm)
{
	UINT32 code = 0, temp;
	unsigned len = strlen(prm);
	unsigned i;

	for (i = 0; i < len - 3; i += 4)
	{
		temp = *(UINT32*)(prm + i);
		temp = switch_endianness(temp);
		code += temp;
	}
